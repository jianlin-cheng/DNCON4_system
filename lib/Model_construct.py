...
def DeepResnet_with_paras_2D(kernel_size,feature_2D_num, filters,nb_layers,opt, initializer = "he_normal", loss_function = "weighted_BCE", weight_p=1.0, weight_n=1.0):
    ...
    # DNCON4_2D_conv = MaxoutConv2D(kernel_size=(1,1), output_dim=64, nb_features=4)(DNCON4_2D_conv)
...
def DeepResnet_with_paras_2D_Pre(kernel_size,feature_2D_num, filters,nb_layers,opt, initializer = "he_normal", loss_function = "binary_crossentropy"):
    ...
    if nb_layers == 46:
        ...
        for i, r in enumerate(repetitions):
            block = _residual_block(basic_block_pre, filters=filters, repetitions=r, is_first_layer=(i == 0))(block)
    elif nb_layers == 152:
        repetitions = [3, 8, 36, 3]
        for i, r in enumerate(repetitions):
            block = _residual_block(basic_block_pre, filters=filters, repetitions=r, is_first_layer=(i == 0))(block)
...
def DeepResnet_with_paras_2D_other(kernel_size,feature_2D_num, filters,nb_layers,opt, initializer = "he_normal", loss_function = "binary_crossentropy"):
...
def DilatedRes_with_paras_2D(kernel_size,feature_2D_num, filters,nb_layers,opt, initializer = "he_normal", loss_function = "weighted_BCE", weight_p=1.0, weight_n=1.0):
    contact_feature_num_2D=feature_2D_num
    contact_input_shape=(None,None,contact_feature_num_2D)
    contact_input = Input(shape=contact_input_shape)
    ...
    DNCON4_2D_conv = _conv_in_relu_K(filters=filters, kernel_size=7, strides=1)(DNCON4_2D_conv)
...
def WeightLearn_with_paras_2D(kernel_size,feature_2D_num, filters,nb_layers,opt, initializer = "he_normal", loss_function = "binary_crossentropy", weight_p=1.0, weight_n=1.0):
    contact_feature_num_2D=feature_2D_num
    contact_input_shape=(None,None,contact_feature_num_2D)
    DNCON4_2D_input = Input(shape=contact_input_shape)

    x1 = Lambda(lambda x: tf.slice(x, [0,0,0,0], [-1,-1,-1,1]))(DNCON4_2D_input)
    x2 = Lambda(lambda x: tf.slice(x, [0,0,0,1], [-1,-1,-1,1]))(DNCON4_2D_input)
    x3 = Lambda(lambda x: tf.slice(x, [0,0,0,2], [-1,-1,-1,1]))(DNCON4_2D_input)
    # x4 = Lambda(lambda x: tf.slice(x, [0,0,0,3], [-1,-1,-1,1]))(DNCON4_2D_input)
    avg = average([x1, x2, x3])
    # max1 = Lambda(lambda x: K.max(x, axis=-1, keepdims=True))(DNCON4_2D_input)
    print(avg.shape.as_list())

    # x1 = Conv2D(1, 1, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(x1)
    # x2 = Conv2D(1, 1, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(x2)
    # x3 = Conv2D(1, 1, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(x3)
    # x1 = Dense(1)(x1)
    # x2 = Dense(1)(x2)
    # x3 = Dense(1)(x3)
    x1 = _conv_bn_relu_K(filters=1, kernel_size=1, strides=1)(x1)
    x2 = _conv_bn_relu_K(filters=1, kernel_size=1, strides=1)(x2)
    x3 = _conv_bn_relu_K(filters=1, kernel_size=1, strides=1)(x3)
    avg = Dense(1)(avg)
    # avg = _conv_in_relu2D(1, 1, 1, strides=(1, 1), use_bias=False, kernel_initializer = "he_normal",  kernel_regularizer=None, dilation_rate=(1,1))(avg)
    # max1 = _conv_in_relu2D(1, 1, 1, strides=(1, 1), use_bias=False, kernel_initializer = "he_normal",  kernel_regularizer=None, dilation_rate=(1,1))(max1)
    
    DNCON4_2D_conv = add([x1, x2, x3])
    DNCON4_2D_conv = _conv_in_relu2D(4, 3, 3, strides=(1, 1), use_bias=False, kernel_initializer = "he_normal",  kernel_regularizer=None, dilation_rate=(1,1))(DNCON4_2D_conv)
    DNCON4_2D_conv = add([DNCON4_2D_conv, avg])

    DNCON4_2D_out = _conv_in_sigmoid2D(filters=1, nb_row=1, nb_col=1, strides=(1, 1), use_bias=False, kernel_initializer=initializer, dilation_rate=(1, 1))(DNCON4_2D_conv)

    loss = loss_function
    DNCON4_WEIGHT = Model(inputs=DNCON4_2D_input, outputs=DNCON4_2D_out)
    DNCON4_WEIGHT.compile(loss=loss, metrics=['accuracy'], optimizer=opt)
    DNCON4_WEIGHT.summary()
    return DNCON4_WEIGHT

def WeightLearn_with_paras_2D_bestBK(kernel_size,feature_2D_num, filters,nb_layers,opt, initializer = "he_normal", loss_function = "binary_crossentropy", weight_p=1.0, weight_n=1.0):
    contact_feature_num_2D=feature_2D_num
    contact_input_shape=(None,None,contact_feature_num_2D)
    DNCON4_2D_input = Input(shape=contact_input_shape)
    ...
    x1 = _conv_in_relu2D(1, 1, 1, strides=(1, 1), use_bias=False, kernel_initializer = "he_normal",  kernel_regularizer=None, dilation_rate=(1,1))(x1)
    x2 = _conv_in_relu2D(1, 1, 1, strides=(1, 1), use_bias=False, kernel_initializer = "he_normal",  kernel_regularizer=None, dilation_rate=(1,1))(x2)
    x3 = _conv_in_relu2D(1, 1, 1, strides=(1, 1), use_bias=False, kernel_initializer = "he_normal",  kernel_regularizer=None, dilation_rate=(1,1))(x3)
    avg = _conv_in_relu2D(1, 1, 1, strides=(1, 1), use_bias=False, kernel_initializer = "he_normal",  kernel_regularizer=None, dilation_rate=(1,1))(avg)
    
    DNCON4_2D_conv = add([x1, x2, x3])
    DNCON4_2D_conv = _conv_in_relu2D(1, 1, 1, strides=(1, 1), use_bias=False, kernel_initializer = "he_normal",  kernel_regularizer=None, dilation_rate=(1,1))(DNCON4_2D_conv)
    DNCON4_2D_conv = add([DNCON4_2D_conv, avg])
    DNCON4_2D_out = _conv_in_sigmoid2D(filters=1, nb_row=1, nb_col=1, strides=(1, 1), use_bias=False, kernel_initializer=initializer, dilation_rate=(1, 1))(DNCON4_2D_conv)
