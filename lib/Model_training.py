...
def DNCON4_1d2dconv_train_win_filter_layer_opt_fast_2D_generator(feature_num,CV_dir,feature_dir,model_prefix,
    ...
    initializer = "he_normal", loss_function = "binary_crossentropy", runcount=1.0,  list_sep_flag=False,  if_use_binsize = False, if_use_generator=True): 
    ...
    elif model_prefix == 'DNCON4_WEIGHT':
    ...
    for epoch in range(epoch_rerun,epoch_outside):
        ...
        if if_use_generator == True:
            ...
            if epoch == 0 and rerun_flag == 0:
                first_inepoch = 5
                history = DNCON4.fit(X_train, Y_train, epochs = first_inepoch)         
                train_loss_list.append(history.history['loss'][first_inepoch-1])
            else: 
                history = DNCON4.fit(X_train, Y_train, epochs = 1)         
                train_loss_list.append(history.history['loss'][first_inepoch-1])           
...
def DNCON4_train_2D_generator(feature_num,CV_dir,feature_dir, model_prefix, epoch_outside,epoch_inside,epoch_rerun,win_array,
    ...
    initializer = "he_normal", loss_function = "weighted_BCE", runcount=1.0, list_sep_flag=False,  if_use_binsize = False): 
    ...
    if model_prefix == 'DNCON4_2dDIARES':
        opt = Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.00001)#0.001  decay=0.0
        DNCON4_RES = DilatedRes_with_paras_2D(win_array,feature_2D_num,nb_filters,nb_layers,opt,initializer,loss_function)
    elif model_prefix == 'DNCON4_2dRES':
        opt = Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.00001)#0.0
        DNCON4_RES = DeepResnet_with_paras_2D(win_array,feature_2D_num,nb_filters,nb_layers,opt,initializer,loss_function)
    ...
    for epoch in range(epoch_rerun,epoch_outside):
        ...
        val_acc_history_content_cov = "%i\t%s\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\n" % (epoch,'COV',
        ...
        val_acc_history_content_plm = "%i\t%s\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\n" % (epoch,'PLM',
        ...
        val_acc_history_content_sum = "%i\t%s\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\n" % (epoch,'SUM',
