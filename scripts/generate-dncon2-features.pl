...
my $psipred_fname     = $ARGV[5];
my $psisolv_fname     = $ARGV[6];
my $ccmpred_fname     = $ARGV[7];
my $freecontact_fname = $ARGV[8];
my $nncon_fname = $ARGV[9];
my $betacon_fname = $ARGV[10];
...
my %con_nncon = %{all_zero_2D_features(0)};
my %con_betacon = %{all_zero_2D_features(0)};
