...
if (not $fasta or not -f $fasta){
	...
	print "Usage: $0 <DB> <tools_dir> <fasta> <output-directory>\n";
	exit(1);
}
...
use constant{
	...
	RRPREDSCRIPT      => abs_path(dirname($0)).'/run-ccmpred-freecontact.pl',
...
confess "Oops!! BLASTNRDB Path not found at ".$db_tool_dir.BLASTNRDB     if not -d $db_tool_dir.BLASTNRDB;
...
if (count_lines("alignment/$id.aln") <= 0){
	print "Too few sequences in the alignment! Skipping CCMpred and FreeContact run..\n";
}
elsif (-s "ccmpred/$id.ccmpred"){
	print "Looks like contact predictions is already here.. skipping..\n";
}
else{
	print "\nRunning CCmpred, and FreeContact parallely..\n";
	system_cmd(RRPREDSCRIPT." $db_tool_dir alignment/$id.aln ccmpred freecontact");
}
...
if (-s "X-$id.txt"){
	...
	system_cmd(GENFEAT. " ./$fasta ./pssm/$id.pssm ./ss_sa/$id.ss_sa ./alnstat/$id.colstats ./alnstat/$id.pairstats ./psipred/$id.ss2 ./psipred/$id.solv ./ccmpred/$id.ccmpred ./freecontact/$id.freecontact.rr ./nncon/${id}_fasta.cm8a ./betacon/${id}_fasta.cm8a.comb > X-$id.txt");
}
...
sub generate_pssm{
	...
	#system_cmd($db_tool_dir.BLASTPATH. "/psiblast -query ".$FASTA." -evalue .001 -inclusion_ethresh .002 -db ".$db_tool_dir.BLASTNRDB. "/$DB -num_iterations 3 -outfmt 0 -out $REPORT -seg yes -out_ascii_pssm $PSSM");
	system_cmd($db_tool_dir.BLASTPATH. "/blastpgp -a 8 -e 0.001 -j 3 -h 0.002 -d ".$db_tool_dir.BLASTNRDB."/$DB -i $FASTA -Q $PSSM >& $REPORT");
}
...
sub generate_pssm_less_stringent{
	...
	my $DB = "nr90";
	print "Running PSI-Blast with $DB...\n";
	#system_cmd($db_tool_dir.BLASTPATH. "/psiblast -query ".$FASTA." -evalue 10 -inclusion_ethresh 10 -db ".$db_tool_dir.BLASTNRDB. "/$DB -num_iterations 3 -outfmt 0 -out $REPORT -num_alignments 2000 -out_ascii_pssm $PSSM");
	system_cmd($db_tool_dir.BLASTPATH. "/blastpgp -a 8 -e 10 -j 3 -h 10 -d ".$db_tool_dir.BLASTNRDB."/$DB -i $FASTA -Q $PSSM >& $REPORT");
}
